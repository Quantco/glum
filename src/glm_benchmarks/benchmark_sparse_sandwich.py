...
def run_one_problem_all_methods(x, d, include_naive) -> pd.DataFrame:
    x.XT = x.T.tocsc()
...
def main() -> None:
    ...
    row_counts = [int(1e4), int(1e5), int(3e5), int(1e6), int(2e6), int(4e6), int(10e6)]
    benchmarks = []
    x, d = load_data(row_counts[-1])
    for i, n_rows in enumerate(row_counts):
        benchmarks.append(
            run_one_problem_all_methods(x[:n_rows, :].copy(), d[:n_rows].copy(), i == 0)
        )
