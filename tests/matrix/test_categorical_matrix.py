...
from glm_benchmarks.matrix.categorical_matrix import (
    csc_dot_categorical,
    csr_dot_categorical,
)
...
@pytest.mark.parametrize("vec_dtype", [np.float64, np.float32, np.int64, np.int32])
def test_csc_dot_categorical(vec_dtype):
    m = 10
    seed = 0
    np.random.seed(seed)
    cat = np.random.choice(np.arange(4, dtype=int), m)[:, None]
    mat = OneHotEncoder().fit_transform(cat).tocsc()
    vec = np.random.choice(np.arange(4, dtype=vec_dtype), mat.shape[1])
    res = csc_dot_categorical(mat.indices, mat.indptr, vec, mat.shape[0])
    np.testing.assert_allclose(res, mat.dot(vec))
