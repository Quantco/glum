...
@pytest.mark.parametrize(
    "names, R, r, r_feat",
    [
        pytest.param(["term_0"], np.array([[0, 1] + 9 * [0]]), None, None, id="single"),
        pytest.param(
            ["term_0", "term_1"],
...
def test_wald_test_term_name(regression_data, names, R, r, r_feat):
    X, y = regression_data
    X_df = pd.DataFrame(X, columns=[f"col_{i}" for i in range(X.shape[1])])
    ...
    # for now, until the term name PR is merged:
    mdl.term_names = ["term_0", "term_1"] + ["term_2"] * (X.shape[1] - 2)

    feature_names_results = mdl.wald_test_term_name(names, r)

    if r is not None:
        r_feat = np.array(r_feat)  # wald_test_matrix expects an optional numpy array
    matrix_results = mdl.wald_test_matrix(R, r_feat)
